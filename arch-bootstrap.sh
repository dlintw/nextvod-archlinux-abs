...
# vim:et ai sw=2 ts=2
#
# arch-bootstrap.sh: Bootstrap a base Arch Linux system.
#
# Dependencies: coreutils, wget, sed, gawk, tar, gzip, chroot, xz.
# Bug tracker: http://code.google.com/p/sh4twbox/issues
# Contact: Daniel YC Lin <dlin.tw at gmail> for sh4
# Original Author: Arnau Sanchez <tokland@gmail.com> for i686,x86_64
#
# Install:
#
...
#   $ sudo arch-bootstrap -a sh4 -r http://sh4twbox.googlecode.com/files destination_sh4
...
EXTRA_PACKAGES=(coreutils bash grep gawk file tar)
PACKDIR="arch-bootstrap"
#DEFAULT_REPO_URL="http://mirrors.kernel.org/archlinux"
#DEFAULT_ARCH=i686
DEFAULT_REPO_URL="http://sh4twbox.googlecode.com/files"
DEFAULT_ARCH=sh4

configure_pacman() {
  local DEST=$1; local ARCH=$2
  cp "/etc/resolv.conf" "$DEST/etc/resolv.conf" || true
...
usage() {
  stderr "Usage: $(basename "$0") [-a i686|x86_64|sh4] [-r REPO_URL] DEST"
  stderr "eg. $(basename "$0") / # default for sh4twbox"
}
...
main() {
  ...
  if ! test -s "$LIST_HTML_FILE"; then
    ...
    if [ $ARCH = sh4 ] ; then
      fetch -O "$LIST_HTML_FILE" "http://code.google.com/p/sh4twbox/downloads/list?num=1000" ||
  ...
  if [ $ARCH = sh4 -a -x /usr/bin/pacman ] ; then
    LC_ALL=C /usr/bin/pacman --noconfirm --arch $ARCH \
